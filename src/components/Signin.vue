    ...
    const login = async () => {
      if(forgotPassword.value === true) {
        ...
        if(!regExpEmail.test(email.value)) errors.value = "Debes introducir un email válido";
          ...
          errors.value = ''
          if(res.data == 'Usuario no encontrado') errors.value = "El usuario no existe"
          else if (res.data == 'Usuario no confirmado') errors.value = "Esta cuenta aún no ha sido confirmada"
